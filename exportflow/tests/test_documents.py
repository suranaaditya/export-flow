...
class TestDocuments(IntegrationTestCase):
	...
	def test_generate_all_documents(self):
		"""One pass generates every generatable Pending document, skips rows
		already past Pending, and never touches tracked documents."""
		so, customer, _s = self.make_deal()
		shp = self.make_shipment(so, customer, incoterm="CIF")
		# one generatable doc is already drafted — the bulk pass must leave it
		generate_document(instance_of(shp, "Commercial Invoice"))

		res = generate_shipment_documents(shp)
		self.assertGreaterEqual(res["generated"], 1)
		self.assertEqual(res["failed"], 0, res["results"])
		generated_types = {r["document_type"] for r in res["results"] if r["ok"]}
		self.assertNotIn("Commercial Invoice", generated_types, "already-Drafted rows are skipped")

		# a generatable type that was Pending is now Drafted with a file
		pl = frappe.db.get_value(
			"Document Instance",
			{"shipment": shp, "document_type": "Packing List"},
			["status", "file"],
			as_dict=True,
		)
		self.assertEqual(pl.status, "Drafted")
		self.assertTrue(pl.file)
		# tracked documents are never generated by the bulk pass
		self.assertEqual(
			frappe.db.get_value(
				"Document Instance", {"shipment": shp, "document_type": "ADC NOC"}, "status"
			),
			"Pending",
		)

	# ----------------------------------------------- CI → realization automation

	def test_commercial_invoice_auto_creates_realization(self):
		frappe.db.set_single_value("ExportFlow Settings", "auto_create_realization", 1)
		so, customer, _s = self.make_deal(qty=10)
		shp = self.make_shipment(so, customer)
		result = generate_document(instance_of(shp, "Commercial Invoice"))

		rel = frappe.db.get_value(
			"Export Realization",
			{"shipment": shp, "export_invoice": result["document_number"]},
			["name", "status", "customer", "invoice_value", "export_date"],
			as_dict=True,
		)
		self.assertTrue(rel, "a realization shell opens when the Commercial Invoice is generated")
		self.assertEqual(rel.status, "Awaiting Realization")
		self.assertEqual(rel.customer, customer)
		self.assertEqual(flt(rel.invoice_value), 120.0, "Σ shipped qty × SO rate = 10 × 12")
		self.assertFalse(rel.export_date, "no FEMA export date until the shipment departs")

		# idempotent — regenerating the CI does not open a second shell
		generate_document(instance_of(shp, "Commercial Invoice"))
		self.assertEqual(frappe.db.count("Export Realization", {"shipment": shp}), 1)

		# the shell carries the FX rate so the FCY invoice value reports the right
		# INR figure on the dashboards (rate = INR / FCY; book_deal uses rate 83)
		rate = frappe.db.get_value("Export Realization", rel.name, "conversion_rate")
		self.assertEqual(flt(rate), 83.0)

	def test_ci_renumber_repoints_realization(self):
		"""Renumbering the CI after a realization opened must re-point the shell
		(not orphan it and let a regenerate open a duplicate)."""
		frappe.db.set_single_value("ExportFlow Settings", "auto_create_realization", 1)
		so, customer, _s = self.make_deal()
		shp = self.make_shipment(so, customer)
		inv = instance_of(shp, "Commercial Invoice")
		auto_no = generate_document(inv)["document_number"]
		self.assertTrue(frappe.db.exists("Export Realization", {"export_invoice": auto_no}))

		update_document_instance(inv, {"document_number": "MNG/2026/001"})
		self.assertEqual(
			frappe.db.get_value("Export Realization", {"shipment": shp}, "export_invoice"),
			"MNG/2026/001",
			"the shell follows the renumbered CI",
		)
		# regenerating with the new number does not open a duplicate
		generate_document(inv)
		self.assertEqual(frappe.db.count("Export Realization", {"shipment": shp}), 1)

	def test_auto_create_realization_respects_toggle(self):
		frappe.db.set_single_value("ExportFlow Settings", "auto_create_realization", 0)
		try:
			so, customer, _s = self.make_deal()
			shp = self.make_shipment(so, customer)
			generate_document(instance_of(shp, "Commercial Invoice"))
			self.assertEqual(frappe.db.count("Export Realization", {"shipment": shp}), 0)
		finally:
			frappe.db.set_single_value("ExportFlow Settings", "auto_create_realization", 1)

	def test_realization_departure_fills_fema_due(self):
		"""The shell created at CI time gets its FEMA export_date + due date when
		the B/L date is entered (the departure trigger)."""
		frappe.db.set_single_value("ExportFlow Settings", "auto_create_realization", 1)
		so, customer, _s = self.make_deal()
		shp = self.make_shipment(so, customer, mode="Sea")
		generate_document(instance_of(shp, "Commercial Invoice"))
		rel = frappe.db.get_value("Export Realization", {"shipment": shp}, "name")
		self.assertFalse(frappe.db.get_value("Export Realization", rel, "export_date"))

		doc = frappe.get_doc("Export Shipment", shp)
		doc.bl_number = "BL-REL-1"
		doc.bl_date = nowdate()
		doc.save()

		row = frappe.db.get_value(
			"Export Realization", rel, ["export_date", "due_date", "currency"], as_dict=True
		)
		self.assertEqual(getdate(row.export_date), getdate(nowdate()))
		# 15-month FEMA window for a foreign-currency export (18 for INR)
		months = 18 if row.currency == "INR" else 15
		self.assertEqual(getdate(row.due_date), getdate(add_months(nowdate(), months)))
